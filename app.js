...
function 启动() {
  ...
  function 更新当前时间() {
    ...
    元素.当前时间.textContent = 时间格式器.format(现在) + 本书滚动后缀();
  }
...
function 绑定事件() {
  ...
  function 执行自动滚动(当前时间) {
    ...
    if (当前时间 - 本次滚动.上次界面时间 >= 自动滚动界面间隔) {
      ...
      // 已持续时长：随界面节拍刷新
      设置文本(元素.已滚动时间, 格式化滚动小时(当前时间 - 本次滚动.开始时刻));
    }
    if (新位置 >= 最大滚动位置 - 0.5) {
      停止自动滚动('已到文末');
      return;
...
// 第一行（剩余时长）后的括号：今日全部书籍的自动滚动总时长
function 今日滚动后缀() {
  return `（${格式化滚动小时(今日滚动毫秒 + 未入账滚动毫秒)}）`;
}

// 第二行（当前时间）后的括号：当前这本书累计的自动滚动时长
function 本书滚动后缀() {
...
function 更新滚动块文本({ 轨道, 最大滚动位置, 进度 }) {
  ...
  设置文本(元素.剩余滚动时间, 格式化剩余滚动时间(剩余秒数) + 今日滚动后缀());
