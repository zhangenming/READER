...
let 引文边框启用 = true;
let 当前字体标签 = '引号内';
// 自动滚动时长统计：今日总时长跨书累计、按本地日期归零；每本书各自累计。
let 今日滚动日期 = '';
let 今日滚动毫秒 = 0;
const 书籍滚动毫秒 = new Map();
let 未入账滚动毫秒 = 0; // 滚动会话中已累计、待结转到统计的毫秒数
...
function 读取正文排版() {
  ...
  // 正文行有左留白（折行句竖条区，见 styles.css --正文左留白），
  // 排版内容宽度必须同样折减，避免折行时末字被 overflow: hidden 裁掉。
  ...
  function 创建行片段(创建起点, 创建终点) {
    ...
    for (let idx = 创建起点; idx < 创建终点; idx += 1) {
      ...
      for (let 边界idx = 0; 边界idx < 片段边界.length - 1; 边界idx += 1) {
        ...
        if (
          '已经但是却又而且虽然所以如果也则乃既甚最更太很还着仍只才就便连或因其之把被者应该必仅刚正每在对为跟均使'.includes(
            字文本,
          )
        ) {
  ...
  function 绘制单列指示器(画布, 上下文, 关键词, 类型) {
    ...
    if (!底图) {
      ...
      if (状态.指示器缓存.段缓存.size !== 原刻度缓存数) {
        const 刻度 = 状态.指示器缓存.段缓存.get(关键词.id);
        console.info('[阅读器] 关键词指示器已重建', {
          类型,
          关键词: 关键词.文本,
          命中数: 关键词.命中位置.length,
          轨道像素高: 画布高,
          密度参照: Number(刻度.密度参照.toFixed(1)),
          耗时毫秒: Math.round(performance.now() - 开始时间),
        });
      }
    }
...
function 今日滚动后缀() {
  return `（${格式化滚动小时(今日滚动毫秒 + 未入账滚动毫秒)}）`;
}

// 第二行（剩余时长）后的括号：当前这本书累计的自动滚动时长
function 本书滚动后缀() {
  const 总毫秒 = (书籍滚动毫秒.get(状态.文件名) ?? 0) + 未入账滚动毫秒;
  return `（${格式化滚动小时(总毫秒)}）`;
}
...
// 把滚动会话中累计的未入账时长结转到今日统计与当前书的统计（跨天先归零）
function 结转未入账滚动毫秒() {
  if (!状态.文件名 || 未入账滚动毫秒 <= 0) {
    return;
  }
  ...
  书籍滚动毫秒.set(
    状态.文件名,
    (书籍滚动毫秒.get(状态.文件名) ?? 0) + 未入账滚动毫秒,
  );
  未入账滚动毫秒 = 0;
}

// 启动时从持久化数据恢复统计：今日总时长与每本书的累计时长
function 载入自动滚动统计() {
  let 数据 = null;
  try {
    数据 = 读取持久化数据();
  ...
  if (
    ...
    typeof 统计 === 'object' &&
    typeof 统计.日期 === 'string' &&
    typeof 统计.今日毫秒 === 'number' &&
    Number.isFinite(统计.今日毫秒)
  ) {
    今日滚动日期 = 统计.日期;
    今日滚动毫秒 = Math.max(0, 统计.今日毫秒);
  }
...
function 读取滚动条度量(轨道高度, 容器高度, 滚动高度) {
...
function 创建指示器底图(画布宽, 画布高, 像素比, 主关键词, 类型, 视口度量) {
  ...
  const 位置脊线宽度 = Math.min(
    画布宽,
    Math.max(1, Math.round(指示器位置脊线宽度 * 像素比)),
  );
  const 最大密度宽度 = 画布宽 - 位置脊线宽度;
  if (最大密度宽度 > 0) {
    底图上下文.globalAlpha = 0.58;
    for (let 像素idx = 0; 像素idx < 刻度.密度比例列表.length; 像素idx += 1) {
      const 密度比例 = 刻度.密度比例列表[像素idx];
      if (密度比例 <= 0) {
        continue;
      }
      const 密度宽度 = Math.max(1, Math.round(最大密度宽度 * 密度比例));
      底图上下文.fillRect(
        画布宽 - 位置脊线宽度 - 密度宽度,
        像素idx,
        密度宽度,
  ...
  // 右缘细线只表达精确命中位置；左侧轮廓的宽度表达局部密度，两种信息不会互相淹没。
  const 段列表 = 刻度.刻度列表;
  底图上下文.globalAlpha = 0.9;
...
function 创建密度轮廓(密度列表, 像素比) {
  const 像素数 = 密度列表.length;
  const 窗口半径 = Math.max(1, Math.round((指示器密度窗口高度 * 像素比) / 2));
  const 完整窗口像素数 = 窗口半径 * 2 + 1;
  const 前缀和 = new Float64Array(像素数 + 1);
  for (let idx = 0; idx < 像素数; idx += 1) {
    前缀和[idx + 1] = 前缀和[idx] + 密度列表[idx];
  }

  const 局部密度列表 = new Float32Array(像素数);
  for (let idx = 0; idx < 像素数; idx += 1) {
    const 起点 = Math.max(0, idx - 窗口半径);
    const 终点 = Math.min(像素数, idx + 窗口半径 + 1);
    const 实际窗口像素数 = 终点 - 起点;
    局部密度列表[idx] =
      ((前缀和[终点] - 前缀和[起点]) * 完整窗口像素数) / 实际窗口像素数;
  }

  const 排序密度 = Array.from(局部密度列表).sort(function 排序数值(左值, 右值) {
    return 左值 - 右值;
  });
  const 分位idx = Math.min(
    排序密度.length - 1,
    Math.floor((排序密度.length - 1) * 指示器密度分位),
  );
  const 密度参照 = Math.max(指示器密度最低参照, 排序密度[分位idx] ?? 0);
  const 密度比例列表 = new Float32Array(像素数);
  for (let idx = 0; idx < 像素数; idx += 1) {
    密度比例列表[idx] = Math.min(1, 局部密度列表[idx] / 密度参照);
  }
  return { 密度比例列表, 密度参照 };
}

/* 刻度与密度只跟版面与命中有关，与谁是主关键词无关，可以跨悬停切换复用。 */
function 读取指示器刻度(关键词, 画布高, 最小刻度高度, 视口度量) {
...
function 创建指示器刻度(关键词, 画布高, 最小刻度高度, 视口度量) {
...
function 保存持久化状态() {
  ...
  const 持久化数据 = 读取持久化数据();
  持久化数据.当前文件名 = 状态.文件名;
  持久化数据.自动滚动统计 = { 日期: 今日滚动日期, 今日毫秒: 今日滚动毫秒 };
  持久化数据.文本状态[状态.文件名] = {
    文件名: 状态.文件名,
    文本长度: 状态.文本.length,
    ...阅读位置,
    总滚动毫秒: 书籍滚动毫秒.get(状态.文件名) ?? 0,
    当前关键词id: 状态.当前关键词id,
    关键词排序: 状态.关键词排序,
    关键词面板展开: 状态.关键词面板展开,
    自动滚动速度: 状态.自动滚动速度,
