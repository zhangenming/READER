...
function 绑定事件() {
  ...
  // 密度自适应目标速度：把当前视口覆盖的文本范围内的句段负担，与「全文平均密度
  // × 视口高度」的期望负担比较——实际负担越高（长句多、文字密）目标越低（慢读），
  // 越低（对话、短句、空白多）目标越高（快读）。二分 + 前缀和 O(log 段数)，
  ...
  function 计算密度自适应目标速度(浮点位置) {
    ...
    const 顶部行idx = Math.max(
      0,
      Math.min(行数 - 1, Math.floor(浮点位置 / 状态.行高)),
    );
    const 视口行数 = Math.max(
      1,
      Math.ceil(元素.滚动容器.clientHeight / 状态.行高) + 1,
    );
    const 结束行idx = Math.min(行数, 顶部行idx + 视口行数);
    const 视口起点 = 状态.行起点列表[顶部行idx];
    ...
    const 视口高度像素 = (结束行idx - 顶部行idx) * 状态.行高;
