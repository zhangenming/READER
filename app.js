...
async function 应用文本(原始文本, 文件名, 全文单字, 载入仍然有效) {
  ...
  // 阶梯段落断点扫描：找出每个「短句段起点」及其阶梯层级。
  // 只记录数据（起点偏移升序 + 层级），不改写文本；是否生效由创建行索引决定。
  // 规则：句内停顿标点（，、；及半角 , ;）之后断行，层级 +1；
  // 句末标点（整理句子换行已为其插入真实换行）与段落起点层级归 0；
  // 引文区域内的停顿不断句（与句子整理的引文策略一致），推迟到区域闭合后断行。
  async function 创建阶梯断点索引(全文, 原引文边界列表, 任务仍然有效) {
    ...
    for (let idx = 0; idx < 全文.length;) {
      ...
      if (停顿标点集合.has(字) || 句末标点集合.has(字)) {
        let 标点终点 = idx;
        let 簇含句末 = false;
        while (标点终点 < 全文.length) {
          const 簇字 = 全文[标点终点];
          if (停顿标点集合.has(簇字) || 句末标点集合.has(簇字)) {
            if (句末标点集合.has(簇字)) {
              簇含句末 = true;
            }
            标点终点 += 1;
