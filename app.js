...
const 自动滚动快速速度 = 2400;
const 自动滚动缓动时长 = 140;
const 自动滚动界面间隔 = 50; // 仅限制布局度量、虚拟行和状态文字，不限制滚动帧率
...
async function 创建Uint8Array(数组, 任务仍然有效) {
...
function 设置内置字词颜色(颜色, 选项 = {}) {
  if (!/^#[\da-f]{6}$/i.test(颜色)) {
    throw new TypeError('内置字词颜色格式无效');
  }
  内置字词颜色 = 颜色.toLowerCase();
  document.documentElement.style.setProperty('--内置字词颜色', 内置字词颜色);
  元素.内置字词颜色选择器.value = 内置字词颜色;
  if (!选项.静默) {
    安排保存持久化状态();
  }
  console.info('[阅读器] 已设置内置字词颜色', { 颜色: 内置字词颜色 });
}
...
const 界面墨色深纸亮度上限 = 0.179; // 纸面更暗时浅墨对比度反超深墨，改用浅墨
const 界面墨色浅纸亮度下限 = 0.787; // 纸面更亮时默认次要墨色对比度仍 ≥ 3:1

function 计算相对亮度(十六进制颜色) {
  return 十六进制颜色
    .match(/[\da-f]{2}/gi)
...
async function 应用文本(原始文本, 文件名, 全文单字, 载入仍然有效) {
  ...
  阶段开始时间 = performance.now();
  const 阶梯断点 = await 创建阶梯断点索引(文本, 缩进起点集合, 载入仍然有效);
  ...
  // 规则：同一段原文之内阶梯持续爬升——句内停顿（，、；：及半角 , ; :）、
  // 句末标点、引文内部句读、引文闭引号之后的正文，都另起一段并 +1 级；
  // 只有原文段落起点（缩进起点集合）才归 0 重新开始。
  // 整理句子换行插入的换行不是段落边界：换行分支在此补记 +1 级，
  // 使引文/句子之后的正文继续爬升。标点簇后的闭引号随前一段一起断行。
  async function 创建阶梯断点索引(全文, 缩进起点集合, 任务仍然有效) {
...
function 重建行索引(排版 = 读取正文排版()) {
  ...
  const 本次阶梯断点 = 阶梯段落启用 ? 状态.阶梯断点 : null;
  const 开始时间 = performance.now();
...
async function 创建行索引(文本, 排版, 缩进起点集合, 阶梯断点, 任务仍然有效) {
  const 起点数组 = [];
  const 终点数组 = [];
  const 逻辑行数组 = [];
  const 段落索引数组 = [];
  ...
  function 完成自动行(终点) {
    添加行(行起点, 终点);
    行起点 = 终点;
    // 阶梯段落：同一段落的折行延续行保持同级缩进（块状缩进），宽度同步扣减。
    当前行宽度 = 当前行阶梯层 * 阶梯步进宽;
    当前行有内容 = false;
  }
...
function 今日本书滚动后缀() {
  const 今日毫秒 = (今日书籍滚动毫秒.get(状态.文件名) ?? 0) + 未入账滚动毫秒;
  return `（${格式化滚动小时(今日毫秒)}）`;
}
...
function 载入自动滚动统计() {
  ...
  if (
    ...
    if (统计.书籍毫秒 !== undefined) {
      if (
        !统计.书籍毫秒 ||
        typeof 统计.书籍毫秒 !== 'object' ||
        Array.isArray(统计.书籍毫秒)
      ) {
        throw new TypeError('持久化的今日按书滚动统计格式无效');
      }
      for (const [文件名, 毫秒] of Object.entries(统计.书籍毫秒)) {
...
function 保存持久化状态() {
  ...
  持久化数据.自动滚动统计 = {
    日期: 今日滚动日期,
    今日毫秒: 今日滚动毫秒,
    书籍毫秒: Object.fromEntries(今日书籍滚动毫秒),
  };
  持久化数据.文本状态[状态.文件名] = {
    文件名: 状态.文件名,
    文本长度: 状态.文本.length,
