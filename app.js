...
function 绑定事件() {
  ...
  function 计算密度自适应目标速度(浮点位置) {
    const 行数 = 状态.行起点列表.length;
    if (行数 === 0 || 状态.句段起点列表.length === 0) {
      return 状态.自动滚动速度;
    }
    const 窗口起点位置 =
      浮点位置 + 元素.滚动容器.clientHeight * 自适应窗口下移比例;
    ...
    const 视口行数 = Math.max(
      1,
      Math.ceil(元素.滚动容器.clientHeight / 状态.行高) + 1,
    );
  ...
  function 执行自动滚动(当前时间) {
    ...
    const 密度目标速度 = 计算密度自适应目标速度(本次滚动.浮点位置);
    const 目标速度 = 快速滚动中
      ? 自动滚动快速速度 * 本次滚动.快速滚动方向
      : 停留中
        ? 0
        : 密度目标速度;
    // 供滚动进度区展示「实际速度」（经窗口密度调整后的目标速度）
    本次滚动.密度目标速度 = 密度目标速度;
    ...
    if (当前时间 - 本次滚动.上次界面时间 >= 自动滚动界面间隔) {
      ...
      // 实际速度 = 基础速度经窗口内容密度调整后的目标速度（逐帧写于会话）。
      元素.基础速度显示.textContent = `基 ${Math.round(状态.自动滚动速度)}`;
      元素.实际速度显示.textContent = `实 ${Math.round(本次滚动.密度目标速度)}`;
    }
    if (新位置 >= 最大滚动位置 - 0.5) {
      停止自动滚动('已到文末');
      return;
...
function 更新自动滚动速度() {
  ...
  // 自适应开启时给出一次性说明；强度为 0 时保持简洁
  if (自适应滚动强度 > 0 && !元素.自动滚动按钮.title) {
    元素.自动滚动按钮.title =
      '自动滚动 · 滚轮或快/慢调整基准速度；速度会按内容密度自动适配（密集放慢、稀疏加快）；点击切换全屏';
  }
}
...
function 应用文本(原始文本, 文件名) {
  ...
  状态.全文单字 = new Set(
    [...汉字频次].filter(function 筛选全文单字([, 频次]) {
      return 频次 === 1;
...
function 读取正文排版() {
  ...
  return {
    ...
    内容宽度,
    正文字号,
    // 段首标记占用一个字宽和少量字距，测量宽度与 CSS 视觉标记保持一致。
    句首缩进: 正文字号 * 1.2,
    行高,
...
function 更新滚动块() {
  ...
  元素.剩余滚动时间.textContent = 格式化剩余滚动时间(剩余秒数);
