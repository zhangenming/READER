...
import { 获取关键词配色, 查找首个不小于的命中, 查找首个相交命中 } from './搜索.js';
import { 更新滚动块, 读取滚动条度量, 轨道中心转滚动位置 } from './滚动条.js';

// 画布上下文延迟到 初始化指示器（app.js 启动时）创建，模块加载本身不触碰 DOM，
// 使本模块在无浏览器环境下可被静态加载。
let 当前指示器上下文 = null;
let 悬停指示器上下文 = null;

export function 初始化指示器() {
  当前指示器上下文 = 元素.关键词指示器.getContext('2d');
  悬停指示器上下文 = 元素.悬停关键词指示器.getContext('2d');
